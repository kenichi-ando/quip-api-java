...
public class QuipTableTest {
	...
	void createMultipleTableInDocument() throws Exception {
		QuipThread doc = QuipThread.createDocument("複数埋め込みテーブルテスト", QuipTable.createTableHtml(2, 3), null, QuipThread.Format.HTML, QuipThread.Type.DOCUMENT);
		doc.editDocument(QuipTable.createTableHtml(1, 2), QuipThread.Format.HTML, null, QuipThread.Location.APPEND);
	...
	void createTableInDocument() throws Exception {
		...
		QuipThread doc = QuipThread.createDocument("埋め込みテーブルテスト", html, null, QuipThread.Format.HTML, QuipThread.Type.DOCUMENT);
		String[] tableIds = doc.getTableIds();
	...
	void createSpreadsheet() throws Exception {
		String html = QuipTable.createTableHtml(
				...
				new String[][] {
						...
						new String[] {"セルA2", "セルB2", "セルC2", "セルD2"},
						new String[] {"セルA3", "セルB3", "セルC3", "セルD3"}});
		QuipThread sheet = QuipThread.createDocument("スプレッドシートテスト", html, null, QuipThread.Format.HTML, QuipThread.Type.SPREADSHEET);
