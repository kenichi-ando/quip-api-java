...
public class QuipFolder extends QuipJsonObject {
    ...
    JsonObject json =
        _getToJsonObject(
            new URIBuilder(QuipAccess.ENDPOINT + "/folders/")
                .addParameter("ids", Stream.of(folderIds).collect(Collectors.joining(",")))
                .build());
  ...
  public static QuipFolder create(String title, Color color, String parentId, String[] memberIds)
      throws Exception {
    Form form = Form.form();
