...
public class QuipClient extends QuipAccess {
	...
	public static void setAccessToken(String clientId, String clientSecret, String redirectUri, String code) throws Exception {
		...
		JsonObject json = _postToJsonObject(new URIBuilder("https://platform.quip.com/1/oauth/access_token")
				.addParameters(params).build());
		_instance._accessToken = json.get("access_token").getAsString();
		_instance._refreshToken = json.get("refresh_token").getAsString();
		if (!_verifyToken())
			throw new IOException("The access token is invalid.");
	}

	public static void refreshAccessToken(String clientId, String clientSecret) throws Exception {
		Objects.requireNonNull(clientId);
		Objects.requireNonNull(clientSecret);
		List<NameValuePair> params = new ArrayList<>();
		params.add(new BasicNameValuePair("client_id", clientId));
		...
		params.add(new BasicNameValuePair("refresh_token", _instance._refreshToken));
		JsonObject json = _postToJsonObject(new URIBuilder("https://platform.quip.com/1/oauth/access_token")
				.addParameters(params).build());
		_instance._accessToken = json.get("access_token").getAsString();
		if (!_verifyToken())
	...
	public static void revokeAccessToken(String clientId, String clientSecret) throws Exception {
		Objects.requireNonNull(clientId);
		Objects.requireNonNull(clientSecret);
		List<NameValuePair> params = new ArrayList<>();
		params.add(new BasicNameValuePair("client_id", clientId));
